...
//								$displayPays = 'none';		
	...
	<!-- ////////////////////////////////// END - FORM ENDS /////////////////////////////////////// -->				
				...
				echo '</div>';
				if($this->Session->read('Role.id') == 3){
					echo $this->BootstrapForm->input('sal_adm_user_id', array(
						'required' => 'required',
						'label' => 'Vendedor:',
						'id'=>'cbxSalesman',
						'selected' => $this->session->read('User.id'),//$admUserId,
						'class'=>'input-xlarge',
						'disabled'=>'disabled'
					));
				}else{
							...
							<tbody>
								...
								for($i=0; $i<$limit2; $i++){
									echo '<tr id="payRow'.$salPayments[$i]['dateId'].'" >';
										...
										if($documentState == 'NOTE_PENDANT' OR $documentState == 'NOTE_APPROVED' OR $documentState == ''){
