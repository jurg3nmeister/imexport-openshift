...
//            "data": data,  //unnecesary, 'cause fill tbody rows with other function
    ...
    function fillTableTbody(data, plotColors, groupId) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (groupId == 0) {
                //html += '<td><a href="/inv_movements/graphics_movements_products/groupName:' + data[i]["label"] + '/groupId:' + data[i]["id"] + '/groupType:' + jQuery("#cbxGroup").val() + '" target="_blank">' + data[i]["label"] + '</a></td>';
                ...
                html += '/month:' + getSelectedAsString("#InvMovementGraphicsMovementsProductsForm #cbxMonth option:selected");
                html += '/location:' + jQuery("#cbxLocation").val();
                html += '/warehouse:' + jQuery("#cbxWarehouse").val();
                html += '/movementType:' + jQuery("#cbxMovementType").val();
